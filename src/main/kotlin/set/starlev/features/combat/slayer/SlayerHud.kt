...
object SlayerHud : HudElement("SlayerHud") {
    ...
    private fun getHudTextColor(): Int = 0xFFFFFFFF.toInt()

    @Suppress("unused")
    private fun parseColor(colorStr: String, default: Int): Int = ColorUtils.parseColor(colorStr, default)
    ...
    override fun render() {
        ...
        if (isEditing && slayerInfo.isEmpty()) {
            ...
            slayerInfo.forEachIndexed { index, line -> if (index > 0) lines.add(line) }
        }
        ...
        val totalHeight = rowHeight * (lines.size + 1)
    ...
    private fun updateSlayerInfo() {
        ...
        if (config.bossTimer) {
            ...
            if (hasKillBossMsg && !isBossTimerRunning) {
                ...
                if (hasBossKilledMsg) handleBossKill()
                isBossTimerRunning = false
            }
    ...
    override fun getWidth(): Int {
        val title = if (isEditing && slayerInfo.isEmpty()) "Zombie T4" else if (currentSlayerType.isNotEmpty()) "$currentSlayerType $currentSlayerTier" else "Slayer"
        val titleWidth = mc.font.width("§lSLAYER §7($title)")
        val lines = if (isEditing && slayerInfo.isEmpty()) listOf("§c☠ Босс появился!") else slayerInfo.drop(1)
        val contentWidth = if (lines.isEmpty()) titleWidth else maxOf(titleWidth, lines.maxOf { mc.font.width(it) })
        return maxOf(contentWidth, 100)
    }

    override fun getHeight(): Int {
        val lines = if (isEditing && slayerInfo.isEmpty()) listOf("dummy") else slayerInfo.drop(1)
    ...
    fun getCurrentSlayerType(): String = currentSlayerType
    fun getCurrentSlayerTier(): String = currentSlayerTier
    fun isBossTimerRunning(): Boolean = isBossTimerRunning
    fun getTimerValue(): Double {
        if (!isBossTimerRunning) return 0.0
        return (System.currentTimeMillis() - bossStartTime) / 1000.0
    }
}
