...
object MegaChestNPCHandler {
    ...
    private val DIALOGUES = listOf(
        ...
        "У меня есть ГИПЕРИОН! А у тебя что?",
        "Если мама удалит мне Роблокс, то я стану фембоем. TwT",
        "СО2 не регает.",
        "Га-ла-тея! Запомнил?",
        "Сколько раз тебе это повторять?!? Га-ла-тея!!!",
        "Я лишь иллюзия, а ты просто Л.",
        "Когда я запустил StarredHeltix.exe, у меня запустилась ратка! Рабочий способ!",
        "Алё, у Лёвы ратка!!!",
        ...
        "Открой мне базу и верни мне бр бр патапима... :C",
        ...
        "Джейд драга и гипу не дам ни кому!",
        "Starlev - главный кринж планеты",
        "Интересный факт: Я не играю в КС, зато играю КС в майнкрафте :P",
        "Я всегда к вашим услугам!",
        "Мега спешит к вам на помощь!",
        "Нет, нет! Это Gala-té-à?",
        "Я люблю Gala-té-à!",
        "Персонаж Пингвин всё так и не отходит от коров...",
        "Пингвин - ... Я боюсь Пингвинов!",
        "Какое сейчас время? А-а, время летать!",
        "Кстати, я есть Deagle Master.",
        "Когда я стану Админом Хелтикс Скайблока, я, в первую очередь, добавлю себя!",
        "Я не Мякиш, не Мага, не Маге, а Ме-га! И я не фиксик.",
        "Когда-нибудь Мега-ящики восстанут человечеству...",
        "Бу-бу-бу! (Пока)",
        "Бе-бе-бе. (я урод)",
        "Лёва когда на Ф7?",
        "МОЙ ХЕЛЬТИК :c",
        "Меня снова хотят забанить на хельтике;(",
        "/gaymode 1 - У вас недостаточно прав. Мега: Вот чёрт!",
        "Обнова галатеи когда?",
        "Когда выйдет Сад, то прийдёт много макросеров с Хайпикселя.",
        "я знаком с ромой дуркой, с никерсом, зуром геймсом и стар шалуном",
        "Никес - просто сникерс!",
        "Лёва устал от Хелтикса, поэтому он и делает мод?",
        "Копаем!",
        "А знаете, этот мир мне больше нравится для цивилизации мега-ящиков."
    )
    ...
    fun handleAttack(): Boolean {
        ...
        if (target != null && target.type == net.minecraft.world.phys.HitResult.Type.ENTITY) {
            ...
            if (entity is GhostMegaChestMagmaCube) {
                ...
                // Звук жителя (счастливый, обычный, злой)
                val soundEvent = when (random.nextInt(3)) {
                    0 -> SoundEvents.VILLAGER_YES
                    1 -> SoundEvents.VILLAGER_AMBIENT
                    else -> SoundEvents.VILLAGER_NO
                }
                mc.level?.playSound(mc.player, entity.x, entity.y, entity.z, soundEvent, net.minecraft.sounds.SoundSource.NEUTRAL, 1.0f, 1.0f)

                val fullMessage = net.minecraft.network.chat.Component.literal("§6§l[МЕГА-ЯЩИК]§f: $message")
                
                // Пытаемся показать через оверлей. Если оверлей вернул true (обработано и скрыто), то не пишем в чат.
                // Если вернул false (оверлей выключен или не обработано), пишем в чат.
                if (!NpcDialogueOverlay.onChat(fullMessage)) {
