...
object CustomBindManager {
    ...
    fun tick() {
        if (!StarredHeltix.feature.misc.customBinds.enabled) return
    ...
    private fun loadFromConfig() {
        binds.clear()
        val config = StarredHeltix.feature.misc.customBinds
        config.binds.forEach { (name, command) ->
            val keyCode = config.keys[name] ?: GLFW.GLFW_KEY_UNKNOWN
    ...
    private fun save() {
        ...
        config.keys.clear()
        binds.forEach { (name, pair) ->
            config.binds[name] = pair.first
            if (pair.second != GLFW.GLFW_KEY_UNKNOWN) {
                config.keys[name] = pair.second
            }
        }
