...
object SpeedBoostCooldownHud : HudElement("SpeedBoostCooldownHud") {
    ...
    private val cooldownMs: Long
        get() = (StarredHeltix.feature.mining.abilityCooldown.speedBoostCooldown.toIntOrNull() ?: 120) * 1000L
    ...
    override fun render() {
        ...
        val text = if (isEditing) "§lС: 120.0" else "§lС: ${String.format("%.1f", remainingTime)}"
        if (isEditing || remainingTime > 0) {
            cachedGraphics?.drawString(mc.font, text, x, y, 0xFF00FF00.toInt())
        }
    }
