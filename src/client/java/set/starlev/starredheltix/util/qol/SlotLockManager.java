...
public class SlotLockManager {
    ...
    private static boolean lockKeyWasPressed = false;
    ...
    public static void register() {
        ...
        lockKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
            "key.starredheltix.slotlock",
            InputUtil.Type.KEYSYM,
            GLFW.GLFW_KEY_L,
            new net.minecraft.client.option.KeyBinding.Category(Identifier.of("starredheltix", "category"))
        ));
        
        // Register tick event to handle key presses
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            // Handle lock mode toggle - работает везде, включая инвентарь
            if (lockKey != null && lockKey.wasPressed()) {
                toggleLockMode();
            }
            
            // Альтернативная проверка L в инвентаре
            if (client.currentScreen instanceof HandledScreen && 
                org.lwjgl.glfw.GLFW.glfwGetKey(client.getWindow().getHandle(), org.lwjgl.glfw.GLFW.GLFW_KEY_L) == org.lwjgl.glfw.GLFW.GLFW_PRESS) {
                if (!lockKeyWasPressed && isSlotLockingEnabled()) {
                    toggleLockMode();
                }
                lockKeyWasPressed = true;
            } else {
                lockKeyWasPressed = false;
            }
        });
    }
    ...
    public static void toggleSlotLock(int slotIndex) {
        ...
        if (lockedSlots.contains(slotIndex)) {
            ...
            if (client.player != null) {
                client.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_CHIME.value(), 0.3f, 1.2f);
            }
        } else {
            lockedSlots.add(slotIndex);
            if (client.player != null) {
                client.player.playSound(SoundEvents.BLOCK_NOTE_BLOCK_PLING.value(), 0.3f, 0.8f);
            }
        }
        saveLockedSlots();
    }
    ...
    private static Slot findHoveredSlotManually(HandledScreen<?> screen) {
        try {
            ...
            List<Slot> slots = getScreenSlots(screen);
            if (slots != null) {
                for (Slot slot : slots) {
    ...
    private static List<Slot> getScreenSlots(HandledScreen<?> screen) {
        try {
            ...
            try {
                ...
                slotsField.setAccessible(true);
                @SuppressWarnings("unchecked")
                List<Slot> slots = (List<Slot>) slotsField.get(handler);
                return slots;
            } catch (Exception e2) {
                if (StarredHeltixClient.CONFIG.general.debugMode) {
                    MinecraftClient client = MinecraftClient.getInstance();
    ...
    public static void setSlotLockEnabled(boolean enabled) {
    ...
    public static boolean isLockKeyPressed() {
        return lockKey != null && lockKey.isPressed();
    }
    
    // Method to check if slot locking feature is enabled
    ...
    public static void toggleLockMode() {
        if (!StarredHeltixClient.CONFIG.slotLocking.slotLockingEnabled) return;
        ...
        if (client.player != null) {
            if (lockModeEnabled) {
                client.player.sendMessage(Text.of("§aРежим блокировки слотов включен"), true);
