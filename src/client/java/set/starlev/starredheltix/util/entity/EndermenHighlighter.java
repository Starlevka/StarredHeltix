...
public class EndermenHighlighter {
    private static final MinecraftClient CLIENT = MinecraftClient.getInstance();
    private static int debugFrameCounter = 0;

    public static void register() {
        System.out.println("EndermenHighlighter: Registering WorldRenderEvents.AFTER_TRANSLUCENT");
        // Using AFTER_TRANSLUCENT instead of BEFORE_ENTITIES for better rendering
        WorldRenderEvents.AFTER_TRANSLUCENT.register(EndermenHighlighter::onWorldRender);
    }
    ...
    private static void onWorldRender(WorldRenderContext context){
        ...
        renderEndermenHighlight(context);
    }

    private static void renderEndermenHighlight(WorldRenderContext context) {
        if (CLIENT.world == null || CLIENT.player == null) return;

        // Check if we should only highlight in the End dimension
        if (StarredHeltixClient.CONFIG.endermenHighlight.endermenHighlightOnlyInEnd) {
            // The End dimension has ID "minecraft:the_end"
            if (!CLIENT.world.getRegistryKey().getValue().toString().equals("minecraft:the_end")) {
                return;
            }
        }
        ...
        int renderedEndermen = 0;
        
        // Render all endermen within extended render distance
        for (Entity entity : CLIENT.world.getEntities()) {
            if (entity instanceof EndermanEntity enderman) {
                // Check if enderman is alive and should be highlighted
                if (enderman.isAlive() && !enderman.isRemoved()) {
                    // Calculate distance squared to avoid expensive sqrt operation
                    double dx = enderman.getX() - cameraPos.x;
                    double dy = enderman.getY() - cameraPos.y;
                    double dz = enderman.getZ() - cameraPos.z;
                    double distanceSq = dx * dx + dy * dy + dz * dz;
                        
                    // Only render if within extended render distance
                    if (distanceSq <= renderDistanceSq) {
                        // Calculate position relative to camera
                        double x = enderman.getX() - cameraPos.x;
                        double y = enderman.getY() - cameraPos.y;
                        double z = enderman.getZ() - cameraPos.z;

                        // Render highlight
                        renderEntityHighlight(context, x, y, z, enderman.getBoundingBox());
                        renderedEndermen++;
                    }
                }
            }
        ...
        if (StarredHeltixClient.CONFIG.general.debugMode && debugFrameCounter++ % 1000 == 0) { // Less frequent debug output
            System.out.println("EndermenHighlighter: Rendered " + renderedEndermen + " endermen");
        }
    }
    ...
    private static void renderBox(WorldRenderContext context, float minX, float minY, float minZ, float maxX, float maxY, float maxZ, float red, float green, float blue, float alpha) {
        ...
        try {
            ...
            System.err.println("EndermenHighlighter: Error rendering box: " + ignored.getMessage());
        }
    }
