...
public class ModUpdater {
    ...
    private static String getCurrentVersion() {
        try {
            ...
            // Try to read from fabric.mod.json in resources
            try (InputStream is = ModUpdater.class.getResourceAsStream("/fabric.mod.json")) {
                if (is != null) {
                    ...
                    String versionValue = json.get("version").getAsString();
                    // Handle the case where version is a placeholder
                    if (versionValue.startsWith("${")) {
                        // Fallback to manifest version
                        return StarredHeltixClient.class.getPackage().getImplementationVersion();
                    }
                    return versionValue;
                }
            } catch (Exception e) {
                System.err.println("Failed to read version from fabric.mod.json: " + e.getMessage());
            }
    ...
    private static boolean isUpdateAvailable(String currentVersion, String latestVersion) {
        // If either version is unknown, we can't determine if an update is needed
        if ("unknown".equals(currentVersion) || "unknown".equals(latestVersion)) {
            return false;
        }
        ...
        // Simple version comparison - split by dots and compare each part numerically
