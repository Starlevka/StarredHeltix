...
public class WaypointCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0, CommandBuildContext var1) {
      var0.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("waypoint").requires(Commands.hasPermission(2))).then(Commands.literal("list").executes((var0x) -> {
      ...
      }))).then(Commands.literal("set").then(Commands.argument("style", ResourceLocationArgument.id()).executes((var0x) -> {
         return setWaypointStyle((CommandSourceStack)var0x.getSource(), WaypointArgument.getWaypoint(var0x, "waypoint"), ResourceKey.create(WaypointStyleAssets.ROOT_ID, ResourceLocationArgument.getId(var0x, "style")));
      })))))));
   }
   ...
   private static int setWaypointColor(CommandSourceStack var0, WaypointTransmitter var1, Integer var2) {
      ...
      var0.sendSuccess(() -> {
         return Component.translatable("commands.waypoint.modify.color", Component.literal(String.format("%06X", ARGB.color(0, var2))).withColor(var2));
      }, false);
      return 0;
   }
