...
public record ClientboundCommandSuggestionsPacket(int id, int start, int length, List<ClientboundCommandSuggestionsPacket.Entry> suggestions) implements Packet<ClientGamePacketListener> {
   ...
   public ClientboundCommandSuggestionsPacket(int var1, Suggestions var2) {
      this(var1, var2.getRange().getStart(), var2.getRange().getLength(), var2.getList().stream().map((var0) -> {
         return new ClientboundCommandSuggestionsPacket.Entry(var0.getText(), Optional.ofNullable(var0.getTooltip()).map(ComponentUtils::fromMessage));
      }).toList());
   }

   public ClientboundCommandSuggestionsPacket(int param1, int param2, int param3, List<ClientboundCommandSuggestionsPacket.Entry> param4) {
      super();
      this.id = var1;
      this.start = var2;
      this.length = var3;
   ...
   public List<ClientboundCommandSuggestionsPacket.Entry> suggestions() {
   ...
   public static record Entry(String text, Optional<Component> tooltip) {
      public static final StreamCodec<RegistryFriendlyByteBuf, ClientboundCommandSuggestionsPacket.Entry> STREAM_CODEC;

      public Entry(String param1, Optional<Component> param2) {
         super();
         this.text = var1;
         this.tooltip = var2;
      }
      ...
      static {
         STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.STRING_UTF8, ClientboundCommandSuggestionsPacket.Entry::text, ComponentSerialization.TRUSTED_OPTIONAL_STREAM_CODEC, ClientboundCommandSuggestionsPacket.Entry::tooltip, ClientboundCommandSuggestionsPacket.Entry::new);
      }
   }
}
