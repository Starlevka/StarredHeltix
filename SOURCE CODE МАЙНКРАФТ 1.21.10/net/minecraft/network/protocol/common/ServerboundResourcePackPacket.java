...
public record ServerboundResourcePackPacket(UUID id, ServerboundResourcePackPacket.Action action) implements Packet<ServerCommonPacketListener> {
   ...
   private ServerboundResourcePackPacket(FriendlyByteBuf var1) {
      this(var1.readUUID(), (ServerboundResourcePackPacket.Action)var1.readEnum(ServerboundResourcePackPacket.Action.class));
   }

   public ServerboundResourcePackPacket(UUID param1, ServerboundResourcePackPacket.Action param2) {
      super();
      this.id = var1;
      this.action = var2;
   }
   ...
   public static enum Action {
      ...
      private static ServerboundResourcePackPacket.Action[] $values() {
         return new ServerboundResourcePackPacket.Action[]{SUCCESSFULLY_LOADED, DECLINED, FAILED_DOWNLOAD, ACCEPTED, DOWNLOADED, INVALID_URL, FAILED_RELOAD, DISCARDED};
      }
   }
}
