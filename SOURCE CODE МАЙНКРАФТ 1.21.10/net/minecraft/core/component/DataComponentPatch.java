...
public final class DataComponentPatch {
   ...
   private static StreamCodec<RegistryFriendlyByteBuf, DataComponentPatch> createStreamCodec(final DataComponentPatch.CodecGetter var0) {
   ...
   public Set<java.util.Map.Entry<DataComponentType<?>, Optional<?>>> entrySet() {
   ...
   static {
      ...
      STREAM_CODEC = createStreamCodec(new DataComponentPatch.CodecGetter() {
         public <T> StreamCodec<RegistryFriendlyByteBuf, T> apply(DataComponentType<T> var1) {
            return var1.streamCodec().cast();
         }
      });
      DELIMITED_STREAM_CODEC = createStreamCodec(new DataComponentPatch.CodecGetter() {
         public <T> StreamCodec<RegistryFriendlyByteBuf, T> apply(DataComponentType<T> var1) {
