...
public class ObjectiveCriteria {
   ...
   static {
      CODEC = Codec.STRING.comapFlatMap((var0) -> {
         return (DataResult)byName(var0).map(DataResult::success).orElse(DataResult.error(() -> {
            return "No scoreboard criteria with name: " + var0;
         }));
   ...
   public static enum RenderType implements StringRepresentable {
      ...
      public static ObjectiveCriteria.RenderType byId(String var0) {
         return (ObjectiveCriteria.RenderType)CODEC.byName(var0, (Enum)INTEGER);
      }
