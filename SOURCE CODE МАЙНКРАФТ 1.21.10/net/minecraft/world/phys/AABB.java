...
public class AABB {
   ...
   public boolean collidedAlongVector(Vec3 var1, List<AABB> var2) {
      ...
      do {
         ...
         var7 = var6.inflate(this.getXsize() * 0.5D - 1.0E-7D, this.getYsize() * 0.5D - 1.0E-7D, this.getZsize() * 0.5D - 1.0E-7D);
   ...
   public double distanceToSqr(AABB var1) {
      double var2 = Math.max(Math.max(this.minX - var1.maxX, var1.minX - this.maxX), 0.0D);
      double var4 = Math.max(Math.max(this.minY - var1.maxY, var1.minY - this.maxY), 0.0D);
      double var6 = Math.max(Math.max(this.minZ - var1.maxZ, var1.minZ - this.maxZ), 0.0D);
      return Mth.lengthSquared(var2, var4, var6);
   }
   ...
   public static class Builder {
      private float minX = 1.0F / 0.0;
      private float minY = 1.0F / 0.0;
      private float minZ = 1.0F / 0.0;
      private float maxX = -1.0F / 0.0;
      private float maxY = -1.0F / 0.0;
      private float maxZ = -1.0F / 0.0;
