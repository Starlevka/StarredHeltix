...
public class NoiseRouterData {
   ...
   private static DensityFunction offsetToDepth(DensityFunction var0) {
      return DensityFunctions.add(DensityFunctions.yClampedGradient(-64, 320, 1.5D, -1.5D), var0);
   }
   ...
   protected static NoiseRouter overworld(HolderGetter<DensityFunction> var0, HolderGetter<NormalNoise.NoiseParameters> var1, boolean var2, boolean var3) {
      ...
      DensityFunction var17 = DensityFunctions.min(var16, DensityFunctions.mul(DensityFunctions.constant(5.0D), getFunction(var0, ENTRANCES)));
      DensityFunction var18 = DensityFunctions.rangeChoice(var16, -1000000.0D, 1.5625D, var17, underground(var0, var1, var16));
      ...
      int var22 = Stream.of(OreVeinifier.VeinType.values()).mapToInt((var0x) -> {
         return var0x.maxY;
      }).max().orElse(-DimensionType.MIN_Y * 2);
      DensityFunction var23 = yLimitedInterpolatable(var20, DensityFunctions.noise(var1.getOrThrow(Noises.ORE_VEININESS), 1.5D, 1.5D), var21, var22, 0);
      float var24 = 4.0F;
      DensityFunction var25 = yLimitedInterpolatable(var20, DensityFunctions.noise(var1.getOrThrow(Noises.ORE_VEIN_A), 4.0D, 4.0D), var21, var22, 0).abs();
      DensityFunction var26 = yLimitedInterpolatable(var20, DensityFunctions.noise(var1.getOrThrow(Noises.ORE_VEIN_B), 4.0D, 4.0D), var21, var22, 0).abs();
      DensityFunction var27 = DensityFunctions.add(DensityFunctions.constant(-0.07999999821186066D), DensityFunctions.max(var25, var26));
   ...
   private static DensityFunction preliminarySurfaceLevel(DensityFunction var0, DensityFunction var1, boolean var2) {
      ...
      DensityFunction var5 = remap(DensityFunctions.add(DensityFunctions.mul(DensityFunctions.constant(0.2734375D), var3.invert()), DensityFunctions.mul(DensityFunctions.constant(-1.0D), var4)), 1.5D, -1.5D, -64.0D, 320.0D);
      var5 = var5.clamp(-40.0D, 320.0D);
      DensityFunction var6 = DensityFunctions.add(slideOverworld(var2, DensityFunctions.add(noiseGradientDensity(var3, offsetToDepth(var4)), DensityFunctions.constant(-0.703125D)).clamp(-64.0D, 64.0D)), DensityFunctions.constant(-0.390625D));
