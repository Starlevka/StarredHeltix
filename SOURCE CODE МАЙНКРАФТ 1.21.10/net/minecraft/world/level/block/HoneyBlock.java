...
public class HoneyBlock extends HalfTransparentBlock {
   ...
   private static double getOldDeltaY(double var0) {
      return var0 / 0.9800000190734863D + 0.08D;
   }

   private static double getNewDeltaY(double var0) {
      return (var0 - 0.08D) * 0.9800000190734863D;
   }
   ...
   private boolean isSlidingDown(BlockPos var1, Entity var2) {
      if (var2.onGround()) {
         ...
         return false;
      } else if (getOldDeltaY(var2.getDeltaMovement().y) >= -0.08D) {
         return false;
      } else {
   ...
   private void doSlideMovement(Entity var1) {
      ...
      if (getOldDeltaY(var1.getDeltaMovement().y) < -0.13D) {
         double var3 = -0.05D / getOldDeltaY(var1.getDeltaMovement().y);
         var1.setDeltaMovement(new Vec3(var2.x * var3, getNewDeltaY(-0.05D), var2.z * var3));
      } else {
         var1.setDeltaMovement(new Vec3(var2.x, getNewDeltaY(-0.05D), var2.z));
      }
