...
public class ArmadilloAi {
   ...
   private static void initCoreActivity(Brain<Armadillo> var0) {
      var0.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new ArmadilloAi.ArmadilloPanic(2.0F), new LookAtTargetSink(45, 90), new MoveToTargetSink() {
         protected boolean checkExtraStartConditions(ServerLevel var1, Mob var2) {
            if (var2 instanceof Armadillo) {
               Armadillo var3 = (Armadillo)var2;
               if (var3.isScared()) {
                  return false;
               }
            }
   ...
   private static void initScaredActivity(Brain<Armadillo> var0) {
      var0.addActivityWithConditions(Activity.PANIC, ImmutableList.of(Pair.of(0, new ArmadilloAi.ArmadilloBallUp())), Set.of(Pair.of(MemoryModuleType.DANGER_DETECTED_RECENTLY, MemoryStatus.VALUE_PRESENT), Pair.of(MemoryModuleType.IS_PANICKING, MemoryStatus.VALUE_ABSENT)));
   }
   ...
   public static class ArmadilloPanic extends AnimalPanic<Armadillo> {
      ...
      // $FF: synthetic method
      protected void start(final ServerLevel param1, final PathfinderMob param2, final long param3) {
