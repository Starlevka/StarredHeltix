...
public class Panda extends Animal {
   ...
   protected void readAdditionalSaveData(ValueInput var1) {
      ...
      this.setMainGene((Panda.Gene)var1.read("MainGene", Panda.Gene.CODEC).orElse(Panda.Gene.NORMAL));
      this.setHiddenGene((Panda.Gene)var1.read("HiddenGene", Panda.Gene.CODEC).orElse(Panda.Gene.NORMAL));
   }
   ...
   class PandaSitGoal extends Goal {
      ...
      public boolean canUse() {
         if (this.cooldown <= Panda.this.tickCount && !Panda.this.isBaby() && !Panda.this.isInWater() && Panda.this.canPerformAction() && Panda.this.getUnhappyCounter() <= 0) {
            if (!Panda.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
               ...
               return !Panda.this.level().getEntitiesOfClass(ItemEntity.class, Panda.this.getBoundingBox().inflate(6.0D, 6.0D, 6.0D), Panda::canPickUpAndEat).isEmpty();
            }
         } else {
            return false;
         }
      ...
      public void start() {
         if (Panda.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
            List var1 = Panda.this.level().getEntitiesOfClass(ItemEntity.class, Panda.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Panda::canPickUpAndEat);
            if (!var1.isEmpty()) {
               Panda.this.getNavigation().moveTo((Entity)var1.getFirst(), 1.2000000476837158D);
            }
         } else {
            Panda.this.tryToSit();
         }
         ...
         if (this.mob.getRandom().nextFloat() >= this.probability) {
            ...
            if (this.lookAt == null) {
               ...
               if (this.lookAtType == Player.class) {
                  ...
                  this.lookAt = var1.getNearestEntity(this.mob.level().getEntitiesOfClass(this.lookAtType, this.mob.getBoundingBox().inflate((double)this.lookDistance, 3.0D, (double)this.lookDistance), (var0) -> {
                     return true;
