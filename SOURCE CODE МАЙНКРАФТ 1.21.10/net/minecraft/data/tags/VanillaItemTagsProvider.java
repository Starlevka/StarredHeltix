...
public class VanillaItemTagsProvider extends IntrinsicHolderTagsProvider<Item> {
   ...
   protected void addTags(HolderLookup.Provider var1) {
      (new BlockItemTagsProvider() {
         protected TagAppender<Block, Block> tag(TagKey<Block> var1, TagKey<Item> var2) {
            return new VanillaItemTagsProvider.BlockToItemConverter(VanillaItemTagsProvider.this.tag(var2));
         }
      }).run();
      ...
      this.tag(ItemTags.PIGLIN_PREFERRED_WEAPONS).add((Object)Items.CROSSBOW);
      ...
      this.tag(ItemTags.TRIM_MATERIALS).addAll(var1.lookupOrThrow(Registries.ITEM).listElements().filter((var0) -> {
         return ((Item)var0.value()).components().has(DataComponents.PROVIDES_TRIM_MATERIAL);
   ...
   private static class BlockToItemConverter implements TagAppender<Block, Block> {
      ...
      public TagAppender<Block, Block> add(Block var1) {
         this.itemAppender.add((Object)((Item)Objects.requireNonNull(var1.asItem())));
         return this;
      }
      ...
      private static TagKey<Item> blockTagToItemTag(TagKey<Block> var0) {
         return TagKey.create(Registries.ITEM, var0.location());
      }
