...
public final class IoSummary<T> {
   ...
   public IoSummary(Duration var1, List<Pair<T, IoSummary.CountAndSize>> var2) {
      ...
      this.totalCountAndSize = (IoSummary.CountAndSize)var2.stream().map(Pair::getSecond).reduce(new IoSummary.CountAndSize(0L, 0L), IoSummary.CountAndSize::add);
   ...
   public static record CountAndSize(long totalCount, long totalSize) {
      ...
      static final Comparator<IoSummary.CountAndSize> SIZE_THEN_COUNT = Comparator.comparing(IoSummary.CountAndSize::totalSize).thenComparing(IoSummary.CountAndSize::totalCount).reversed();

      public CountAndSize(long param1, long param3) {
         super();
