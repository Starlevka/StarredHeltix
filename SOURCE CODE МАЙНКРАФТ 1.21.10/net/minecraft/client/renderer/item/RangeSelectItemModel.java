...
public class RangeSelectItemModel implements ItemModel {
   ...
   public static record Entry(float threshold, ItemModel.Unbaked model) {
      ...
      public static final Codec<RangeSelectItemModel.Entry> CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(Codec.FLOAT.fieldOf("threshold").forGetter(RangeSelectItemModel.Entry::threshold), ItemModels.CODEC.fieldOf("model").forGetter(RangeSelectItemModel.Entry::model)).apply(var0, RangeSelectItemModel.Entry::new);
      });
      ...
      public Entry(float param1, ItemModel.Unbaked param2) {
         super();
         this.threshold = var1;
         this.model = var2;
      }
   ...
   public static record Unbaked(RangeSelectItemModelProperty property, float scale, List<RangeSelectItemModel.Entry> entries, Optional<ItemModel.Unbaked> fallback) implements ItemModel.Unbaked {
      public static final MapCodec<RangeSelectItemModel.Unbaked> MAP_CODEC = RecordCodecBuilder.mapCodec((var0) -> {
         return var0.group(RangeSelectItemModelProperties.MAP_CODEC.forGetter(RangeSelectItemModel.Unbaked::property), Codec.FLOAT.optionalFieldOf("scale", 1.0F).forGetter(RangeSelectItemModel.Unbaked::scale), RangeSelectItemModel.Entry.CODEC.listOf().fieldOf("entries").forGetter(RangeSelectItemModel.Unbaked::entries), ItemModels.CODEC.optionalFieldOf("fallback").forGetter(RangeSelectItemModel.Unbaked::fallback)).apply(var0, RangeSelectItemModel.Unbaked::new);
      });

      public Unbaked(RangeSelectItemModelProperty param1, float param2, List<RangeSelectItemModel.Entry> param3, Optional<ItemModel.Unbaked> param4) {
         super();
         this.property = var1;
         this.scale = var2;
         this.entries = var3;
      ...
      public ItemModel bake(ItemModel.BakingContext var1) {
         ...
         for(int var5 = 0; var5 < var4.size(); ++var5) {
            RangeSelectItemModel.Entry var6 = (RangeSelectItemModel.Entry)var4.get(var5);
      ...
      public List<RangeSelectItemModel.Entry> entries() {
         return this.entries;
      }
