...
public class CloudRenderer extends SimplePreparableReloadListener<Optional<CloudRenderer.TextureData>> implements AutoCloseable {
   ...
   public void render(int var1, CloudStatus var2, float var3, Vec3 var4, float var5) {
      if (this.texture != null) {
         ...
         CloudRenderer.RelativeCameraPos var11;
         if (var10 < 0.0F) {
         ...
         double var14 = var4.z + 3.9600000381469727D;
         double var16 = (double)this.texture.width * 12.0D;
         double var18 = (double)this.texture.height * 12.0D;
         var12 -= (double)Mth.floor(var12 / var16) * var16;
         var14 -= (double)Mth.floor(var14 / var18) * var18;
         int var20 = Mth.floor(var12 / 12.0D);
         int var21 = Mth.floor(var14 / 12.0D);
         ...
         if (this.needsRebuild || var20 != this.prevCellX || var21 != this.prevCellZ || var11 != this.prevRelativeCameraPos || var2 != this.prevType) {
            ...
            this.prevType = var2;
            this.utb.rotate();
            var26 = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.utb.currentBuffer(), false, true);
         ...
         if (this.quadCount != 0) {
            var26 = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.ubo.currentBuffer(), false, true);

            try {
               Std140Builder.intoBuffer(var26.data()).putVec4(ARGB.redFloat(var1), ARGB.greenFloat(var1), ARGB.blueFloat(var1), 1.0F).putVec3(-var22, var9, -var23).putVec3(12.0F, 4.0F, 12.0F);
            ...
            GpuBufferSlice var42 = RenderSystem.getDynamicUniforms().writeTransform(RenderSystem.getModelViewMatrix(), new Vector4f(1.0F, 1.0F, 1.0F, 1.0F), new Vector3f(), new Matrix4f(), 0.0F);
            ...
            RenderPass var33 = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> {
               return "Clouds";
            }, var29, OptionalInt.empty(), var30, OptionalDouble.empty());

            try {
               var33.setPipeline(var25);
               RenderSystem.bindDefaultUniforms(var33);
               var33.setUniform("DynamicTransforms", var42);
