...
public class RecipeButton extends AbstractWidget {
   ...
   public void init(RecipeCollection var1, boolean var2, RecipeBookPage var3, ContextMap var4) {
      ...
      this.selectedEntries = var5.stream().map((var1x) -> {
         return new RecipeButton.ResolvedEntry(var1x.id(), var1x.resultItems(var4));
      }).toList();
   ...
   private static boolean allRecipesHaveSameResultDisplay(List<RecipeButton.ResolvedEntry> var0) {
      Iterator var1 = var0.stream().flatMap((var0x) -> {
         return var0x.displayItems().stream();
      }).iterator();
      if (!var1.hasNext()) {
         return true;
      } else {
         ...
         do {
            ...
            var3 = (ItemStack)var1.next();
         } while(ItemStack.isSameItemSameComponents(var2, var3));
   ...
   public RecipeDisplayId getCurrentRecipe() {
      ...
      return ((RecipeButton.ResolvedEntry)this.selectedEntries.get(var1)).id;
   }
   ...
   public ItemStack getDisplayStack() {
      ...
      return ((RecipeButton.ResolvedEntry)this.selectedEntries.get(var4)).selectItem(var3);
   }
   ...
   static record ResolvedEntry(RecipeDisplayId id, List<ItemStack> displayItems) {
      ...
      ResolvedEntry(RecipeDisplayId param1, List<ItemStack> param2) {
         super();
         this.id = var1;
