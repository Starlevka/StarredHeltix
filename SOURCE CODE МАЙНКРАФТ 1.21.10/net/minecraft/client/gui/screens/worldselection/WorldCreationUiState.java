...
public class WorldCreationUiState {
   ...
   private void updatePresetLists() {
      ...
      this.normalPresetList.addAll((Collection)getNonEmptyList(var1, WorldPresetTags.NORMAL).orElseGet(() -> {
         return var1.listElements().map(WorldCreationUiState.WorldTypeEntry::new).toList();
      }));
      ...
      if (var2 != null) {
         WorldCreationUiState.WorldTypeEntry var3 = (WorldCreationUiState.WorldTypeEntry)findPreset(this.getSettings(), var2.unwrapKey()).map(WorldCreationUiState.WorldTypeEntry::new).orElse((WorldCreationUiState.WorldTypeEntry)this.normalPresetList.getFirst());
