   ...
   private GlyphSource createSprite(final TextureAtlasSprite var1) {
      return new SingleSpriteSource(new BakedGlyph() {
         ...
         public TextRenderable createGlyph(float var1x, float var2, int var3, int var4, Style var5, float var6, float var7) {
            return new AtlasGlyphProvider.Instance(AtlasGlyphProvider.this.renderTypes, AtlasGlyphProvider.this.atlas.getTextureView(), var1, var1x, var2, var3, var4, var7);
         }
      });
   }

   private static record Instance(GlyphRenderTypes renderTypes, GpuTextureView textureView, TextureAtlasSprite sprite, float x, float y, int color, int shadowColor, float shadowOffset) implements PlainTextRenderable {
      Instance(GlyphRenderTypes param1, GpuTextureView param2, TextureAtlasSprite param3, float param4, float param5, int param6, int param7, float param8) {
         super();
